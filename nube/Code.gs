/**
 * MathStep · Ascending Learning — Servidor en Google Sheets (Apps Script)
 *
...
function doGet(e){
  ...
  if(!codigo) return json_({ok:true, app:'MathStep', mensaje:'Servidor activo'});
  ...
  const cuerpo = d
    ? `<div class="ok">✓ Diploma válido</div><p>MathStep certifica que <b>${esc_(d.apodo)}</b> completó la ruta de aprendizaje de matemáticas de 1.º a 11.º.</p>
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
    <title>Verificación de diploma · MathStep</title>
    ...
    p{line-height:1.5}</style></head><body><div class="t"><h1>MathStep · Ascending Learning</h1>${cuerpo}</div></body></html>`;
...
function diploma_(d){
  ...
  const codigo = 'MS-' + Utilities.getUuid().replace(/-/g,'').slice(0,8).toUpperCase();
...
function onOpen(){
  SpreadsheetApp.getUi().createMenu('MathStep')
...
function actualizarPanel(){
  ...
  const filas = Object.keys(stats).map(k => { const s = stats[k];
    ...
    .sort((a, b) => a[4] - b[4]);
  h.clear();
  const resumen = [['Panel de MathStep', ''], ['Actualizado', Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd HH:mm')],
